...
function spkeypress() {
    ...
    cy.log("error revisar linea 450" );
...
function changeViewport() {
    ...
    cy.task("logCommand", {
        funtype: "Viewport change",
        info: `Changed the viewport to ${viewports[index]} with ${orientations[oindex]} orientation`
    })
}
...
function fillInput() { //Or fill form
    cy.window().then((win) => {
        ...
        cy.task("logCommand", {funtype: "Typing into input", info: info})
    })
}
...
function authenticateInGhostApp() {
    cy.task('genericLog', {'message': 'Logging in...'})
    cy.log('Logging in...')
    cy.log("linea 641" );
    cy.get('input[name="identification"]').type('admin@admin.com');
    cy.log("linea 643" );
    cy.get('input[name="password"]').type('AD1m*456Gh0st');
    cy.log("linea 645" );
